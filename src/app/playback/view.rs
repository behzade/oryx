...
impl OryxApp {
    ...
    pub(in crate::app) fn open_visualizer_modal(&mut self, cx: &mut Context<Self>) {
        self.update_ui_state(cx, |state| state.open_visualizer_modal());
        cx.notify();
    }

    pub(in crate::app) fn close_visualizer_modal(&mut self, cx: &mut Context<Self>) {
        let closed = self
            .ui_state
            .update(cx, |state, _cx| state.close_visualizer_modal());
        if closed {
            cx.notify();
        }
    }
